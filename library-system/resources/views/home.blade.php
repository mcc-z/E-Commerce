...
<body class="text-slate-900">
    ...
    <header class="py-16 px-4 text-center">
        ...
        <div class="flex justify-center gap-4">
            <a href="{{ route('login') }}" class="bg-blue-600 text-white px-8 py-4 rounded-xl font-bold inline-flex items-center gap-2 hover:bg-blue-700 transition shadow-lg shadow-blue-200">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
            <div class="text-4xl font-extrabold text-blue-600 mb-2">{{ number_format($stats['books_count']) }}</div>
            <div class="text-slate-500 font-medium">Books Available</div>
        </div>
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
            <div class="text-4xl font-extrabold text-purple-600 mb-2">{{ number_format($stats['members_count']) }}</div>
            <div class="text-slate-500 font-medium">Active Members</div>
        </div>
        <div class="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
            <div class="text-4xl font-extrabold text-indigo-600 mb-2">{{ number_format($stats['borrows_count']) }}</div>
            ...
            <div class="bg-white rounded-2xl overflow-hidden shadow-md border border-slate-100 hover:shadow-xl transition duration-300">
                <div class="h-64 overflow-hidden relative">
                    ...
                    <div class="absolute top-4 right-4 bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold text-blue-600 uppercase tracking-wider">
                        {{ $book->category->name ?? 'Library' }}
                    </div>
                </div>
                <div class="p-6">
                    <h3 class="font-bold text-xl mb-1 text-slate-800 line-clamp-1">{{ $book->title }}</h3>
                    <p class="text-slate-500 mb-4">{{ $book->author }}</p>
                    <div class="flex items-center justify-between pt-4 border-t border-slate-50">
                        <span class="text-sm font-medium {{ $book->available_copies > 0 ? 'text-green-600' : 'text-red-600' }}">
                            {{ $book->available_copies > 0 ? 'Available Now' : 'Out of Stock' }}
                        </span>
                        <a href="{{ route('login') }}" class="text-blue-600 font-bold text-sm hover:text-blue-700">View Details</a>
                    </div>
                </div>
            </div>
            @endforeach
    ...
    <section class="text-center py-20 bg-slate-50">
        ...
        <a href="{{ route('login') }}" class="bg-slate-950 text-white px-10 py-4 rounded-xl font-bold inline-flex items-center gap-3 hover:bg-slate-800 transition">
